...
class DGCNN_backbone(nn.Module):
    def __init__(self, k=20, emb_dims=1024, c_dim=128):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(3 * 2, 64, kernel_size=1, bias=False), self.bn1, nn.LeakyReLU(negative_slope=0.2)
        )
        ...
        self.conv6 = nn.Sequential(
            nn.Conv1d(192, emb_dims, kernel_size=1, bias=False), self.bn6, nn.LeakyReLU(negative_slope=0.2)
        )
        self.conv7 = nn.Sequential(
            nn.Conv1d(1216, 512, kernel_size=1, bias=False), self.bn7, nn.LeakyReLU(negative_slope=0.2)
        )
        self.conv8 = nn.Sequential(
            nn.Conv1d(512, c_dim, kernel_size=1, bias=False), self.bn8, nn.LeakyReLU(negative_slope=0.2)
        )
...
class DGCNN_semseg(nn.Module):
    ...
    def summary(self):
        ...
        #     try:
        #         print(f"\t\t{layer.encoders}")
        #         print(f"\t\t{layer.decoders}")
        #         print(f"\t\t{layer.final_conv}")
        #         print(f"\t\t{layer.final_activation}")
        #     except Exception:
        #         continue
