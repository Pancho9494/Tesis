...
# Arguments
parser = argparse.ArgumentParser(description="Train Implicit Autoencoder.")
...
while True:
    ...
    for batch in train_loader:
        it += 1
        print(f"Epoch[{epoch_it}]Iter[{it}] ", end="")
        loss = trainer.train_step(batch, epoch_it, it)
        ...
        if validate_every > 0 and (it % validate_every) == 0:
            # if 1:
            eval_dict = trainer.evaluate(val_loader, epoch_it, it)
