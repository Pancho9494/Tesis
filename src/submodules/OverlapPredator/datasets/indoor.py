...
class IndoorDataset(Dataset):
    ...
    def __init__(self, infos, config, data_augmentation=True):
        ...
        # self.max_points = 4_096

    def __len__(self):
        return len(self.infos["rot"])

    def __getitem__(self, item):
        src_pcd = torch.load(os.path.join(self.base_dir, self.infos["src"][item]))
        tgt_pcd = torch.load(os.path.join(self.base_dir, self.infos["tgt"][item]))
