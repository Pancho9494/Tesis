...
class MetricLoss(nn.Module):
    ...
    def __init__(self, configs, log_scale=16, pos_optimal=0.1, neg_optimal=1.4):
        ...
        self.log_scale = log_scale                                  # 24
        self.pos_optimal = pos_optimal                              # 0.1
        self.neg_optimal = neg_optimal                              # 1.4

        self.pos_margin = configs.pos_margin                        # 0.1
        self.neg_margin = configs.neg_margin                        # 1.4
        self.max_points = configs.max_points                        # 256

        self.safe_radius = configs.safe_radius                      # 0.1
        self.matchability_radius = configs.matchability_radius      # 0.05
        self.pos_radius = configs.pos_radius                        # 0.0375
    ...
    def forward(
        self, src_pcd, tgt_pcd, src_feats, tgt_feats, correspondence, rot, trans, scores_overlap, scores_saliency
    ):
        """
        ...
        src_saliency_scores = scores_saliency[: src_pcd.size(0)]
        src_saliency_scores = src_saliency_scores[src_idx]
        tgt_saliency_scores = scores_saliency[src_pcd.size(0) :]
        ...
        c_dist = torch.norm(
            input=(
                src_pcd[correspondence[:, 0]] - 
                tgt_pcd[correspondence[:, 1]]
            ),
            dim=1,
        )
        correspondence = correspondence[c_dist < self.pos_radius - 0.001]
