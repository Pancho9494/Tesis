...
static PyObject* batch_subsampling(PyObject* self, PyObject* args, PyObject* keywds)
{
	...
	original_points = std::vector<PointXYZ>((PointXYZ*)PyArray_DATA(points_array), (PointXYZ*)PyArray_DATA(points_array) + N);
	original_batches = std::vector<int>((int*)PyArray_DATA(batches_array), (int*)PyArray_DATA(batches_array) + Nb);
	if (use_feature)
		original_features = std::vector<float>((float*)PyArray_DATA(features_array), (float*)PyArray_DATA(features_array) + N * fdim);
	if (use_classes)
		original_classes = std::vector<int>((int*)PyArray_DATA(classes_array), (int*)PyArray_DATA(classes_array) + N * ldim);
