...
class Pair:
    ...
    def show(self, predicted_tf: np.ndarray | None = None) -> None:
        if predicted_tf is None:
            predicted_tf = np.eye(4, 4)
        ...
        src_pcd = Painter.Uniform(BLUE, compute_normals=True)(
            self.source, to_legacy=False if isinstance(self.source.pcd, o3d.cpu.pybind.geometry.PointCloud) else True
        )
        tgt_pcd = Painter.Uniform(YELLOW, compute_normals=True)(
            self.target, to_legacy=False if isinstance(self.target.pcd, o3d.cpu.pybind.geometry.PointCloud) else True
        )
